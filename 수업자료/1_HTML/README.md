...
2. 시작태그와 종료태그로 구성
3. 종료태그가 없는 태그들(br, img, hr, meta, input) <!-- 경수. 표기방식은? -->
...
4. `<select>` : 콤보 상자 작성
5. `<textarea>` : 텍스트를 열어 줄 입력하는 상자 작성
6. `<botton>` : 버튼 형식의 요소 작성

- form 태그 속성

  - name : form 태그의 이름을 지정
  - method : 데이터 전송 방식을 지정
  - action : 데이터를 전달할 서버(페이지) <!--경수 서버 백앤드를 알아야 해서 이건 나중에-->

  ## 전송 방식 : Get, Post <!--경수 POST는숨겨서하는 방식., GET, POST 중요-->
...
-사용자로부터 데이터를 입력받기 위한 태그 -속성 type의 값에 따라서 input이 변형된다.

1. **text** : 한 줄의 텍스트를 입력받을 수 있다. <!--많이 쓰이는 타입은 입력함-->
2. **passowrd** : 비밀번호를 입력받을 수 있다.
3. **checkbox** : 여러 항목 중 여러 가지를 선택할 수있다.
...
- form 태그의 method 속성에 지정된 전송 방식으로 action 속성에 지정한 서버로 입력한 값들을 전송한다. <!--경수. method는 get, post 두개 였다.-->

8. **reset** : form 택 내에서 입력한 값을 모두 초기화한다.
9. **button** : 기본적으로 가진 기능이 없어 다른 기능과 연결해서 사용한다. <!--경수. 자바스크립트로 구현할때 사용한다라고 말씀하신 듯함-->
...
- **date** : 년/월/일에 대한 데이트로 입력받을 수 있다. <!--경수. 가장 많이 씀-->
- **time** : 시간(시/분/초)에 대한 데이터를 입력받을 수 있다.<!--경수. datetime을 date와 time으로 쪼갰다고 보면 됨-->
...
4. search : 검색 값을 입력받을 수 있다.<!--경수. input type text와 search 차이 보여주겠다.-->
...
## div와 span<!--경수. 의미없으면 div 로 쪼갠다.-->

- HTML에서 주로 사용하는 의미 없는 태그
...
- 단락, 이미지, 제목 등의 콘텐츠를 묶어 더 큰 놀리적 단위 형성
...
- 본문 내용에 대한 추가적인 설명, 부수적인 콘텐츠 표시에 사용하는 태크
...
- 이미지에 대한 캡션이나 설명을 위한 태그 <!--경수. 거의 안쓴다.-->
